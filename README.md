...
To state the objective formally, this project focuses on building a classification model targeted on predicting the occurrence of economic recessions in the U.S. 12-months prior to their onset. Scores output by this model are then used to develop trading rules that aim to shift a theoretical investor's portfolio out of equities before recessions - and the often substantial stock market declines that accompany them - begin. The "stock market" referred to throughout this text is the S&P 500, and any money not invested in the market at any given time is assumed to be invested in 10-year U.S. Treasuries.  
...
![alt text](https://github.com/ErickDWalker/Recession_Prediction/blob/master/img/Logistic_Regression_Output.png)
...
![alt text](https://github.com/ErickDWalker/Recession_Prediction/blob/master/img/portfolio_weights_60:40_return_test.png)

It bears mentioning that while the trading rules successfully reduced the portfolio's equity allocation during the 2008-2009 recession, they failed to do so during the recession starting March 2020, since the model's score never reached the 0.70 "trigger" level. Though this is far from ideal, I think two things are worth noting. First, the fact that the model's score failed to reach the 0.70 level triggering an allocation shift is not all that surprising considering the nature of the current recession. The cause of the decline in GDP in the U.S., and the speed at which it occurred, are unprecedented in the time frame over which the model was trained (and all of U.S. history). This would understandably make it more difficult for the model to make accurate predictions. Second, even with this "miss", the activation of the trading rules during the 2008-2009 recession was enough to lead to appreciable outperformance relative to the passive portfolio over the January, 2003 - May, 2020 window. In other words, the model and the rules derived from it need not be 100% accurate to add value; sidestepping the majority of recessions is enough.  

A 60 | 40 portfolio following the aforementioned trading rules produced a 9.44% CAGR (including dividends) over the test time period, versus the passive 60 | 40 portfolio's 7.77%. Translating that outperformance into dollars, an initial investment of $10,000 in the passive strategy would have grown to $36,634, while the trading portfolio would have grown to $47,779 (plot of the growth of $10,000 in various portfolios shown below). In addition the trading portfolio achieved this outperformance with a lower standard deviation of returns (monthly: 1.87% vs 2.08%).  

![alt text](https://github.com/ErickDWalker/Recession_Prediction/blob/master/img/60:40_test.png)


Conclusion
---
While this is not at all conclusive evidence that active management can reliably outperform its passive counterpart (many factors were not considered, e.g. taxes) I think it does open up a conversation around the debate. To reiterate the point made at the beginning, if it can be shown that economic downturns are to a large degree predictable, then it should be possible to construct trading strategies that take advantage of that predictability, and avoid substantial portions of the stock market's downturns. To lend some academic credence to that end, I note that as early as 1986 economist Campbell Harvey wrote his dissertation on using the yield curve to predict the business cycle. In other words, the concept that recessions are preceded by identifiable patterns in other macroeconomic variables is not new.

Finally, for those concerned projects of this nature might exacerbate short-sightedness among investors, I think it is worth pointing out that a portfolio applying these allocation rules would likely not engage in the kind of frequent trading many disapprove of. Assuming the model's predictive ability remains consistent (or even improves with additional training data) an investor's portfolio would materially shift only twice every economic cycle. To me, this hardly qualifies as the kind of manic trading passive investors so often disparage. Instead, I would argue that this is merely another tool investors can use to decide if it may be prudent to temporarily de-risk their portfolio in anticipation of economic weakness. For many, particularly those most vulnerable to losing their jobs during recessions, this can prove to be a valuable countercyclical adjustment, providing a cash buffer with which to safely navigate economic downturns.
